...
Controller.prototype.handleRequest = function (request, response, next, settings) {
  ...
  if (!request.originalUrl)
    request.originalUrl = this._getOriginalUrl(request);
...
// Get original path if exists
Controller.prototype._getOriginalUrl = function (request) {
  if (!process.env.ORIGINAL_PATH_HEADER)
    return request.parsedUrl;
  else {
    var originalPathHeader = _.kebabCase(process.env.ORIGINAL_PATH_HEADER);
    if (!request.headers[originalPathHeader])
      return request.parsedUrl;
    else {
      var originalPath = request.headers[originalPathHeader];
      var pathname = url.parse(originalPath, true).pathname;
      var search = url.parse(request.url, true).search;
      var path = pathname + search;
      return _.defaults({ path: path, pathname: pathname },
                        request.parsedUrl);
    }
  }
};
